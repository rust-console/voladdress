...
fn test_size_hint_and_next() {
  let s: VolSeries<i32, 3, 16> = unsafe { VolSeries::new(4) };
  let mut i: VolStridingIter<i32, 16> = s.iter();
...
fn test_count() {
  let s: VolSeries<i32, 3, 16> = unsafe { VolSeries::new(4) };
  let i: VolStridingIter<i32, 16> = s.iter();
...
fn test_nth() {
  let s: VolSeries<i32, 3, 16> = unsafe { VolSeries::new(4) };
  let mut i: VolStridingIter<i32, 16> = s.iter();
  let mut i2: VolStridingIter<i32, 16> = i.clone();
